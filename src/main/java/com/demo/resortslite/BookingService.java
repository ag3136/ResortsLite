...
public class BookingService {
    ...
    // Fixed: cz-java-0062 - Externalized payment API endpoint to support GKE deployment
    // BEFORE: private static final String PAYMENT_API = "http://10.0.1.45:9090/payments/charge";
    // AFTER: Use Kubernetes Service DNS for in-cluster communication or ConfigMap for external endpoints
    // The hardcoded IP address (10.0.1.45) has been replaced with a configurable property that:
    // - Defaults to Kubernetes service DNS (payment-service:9090/payments/charge)
    // - Can be overridden via application.properties (app.payment.endpoint)
    // - Can be overridden via environment variable (PAYMENT_SERVICE_URL)
    // This enables GKE Autopilot deployment with dynamic service discovery
    @Value("${app.payment.endpoint:#{systemEnvironment['PAYMENT_SERVICE_URL'] ?: 'http://payment-service:9090/payments/charge'}}")
    ...
    public String generateReport(String month) {
        // Fixed: cz-java-0062 - Updated to use externalized payment API URL
